...
export const thoughtsRepository = {
    async create(dto: CreateThoughtDTO) {
        ...
        await db.runAsync(
            `INSERT INTO ${TABLE} (id, date, situation, thought, emotion, behavior, evidencePro, evidenceContra, alternativeThoughts, reevaluation, deleted_at, synced)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                newRecord.id,
                newRecord.date,
    ...
    async upsert(records: any[]) {
        ...
        for (const record of records) {
            ...
            await db.runAsync(
                `INSERT OR REPLACE INTO ${TABLE} (id, date, situation, thought, emotion, behavior, evidencePro, evidenceContra, alternativeThoughts, reevaluation, deleted_at, synced)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                    record.id,
                    record.date,
    ...
    async softDelete(id: string) {
        const db = getConnection();
        await db.runAsync(
            `UPDATE ${TABLE} SET deleted_at = ?, synced = 0 WHERE id = ?`,
